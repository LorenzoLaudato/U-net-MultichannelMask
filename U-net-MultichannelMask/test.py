...
dir_save_pred_1='./test_outputs/IMAGES'
dir_save_pred_2='./test_outputs/TRUE_MASKS'
dir_save_pred_3='./test_outputs/PRED_MASKS'
    ...
    for i, item in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            for j in range(batch_size):
                ...
                for l in range(7):
                    compact+=one_hot[j,l,:,:].cpu()#+ one_hot[j,1,:,:].cpu()+one_hot[j,2,:,:].cpu()+one_hot[j,3,:,:].cpu()+one_hot[j,4,:,:].cpu()+one_hot[j,5,:,:].cpu()+one_hot[j,6,:,:].cpu()
...
def get_args():
    ...
    parser.add_argument('--load', '-f', type=str, default="/user/llaudato6/Pytorch-UNet-master-2/checkpoints3_8/checkpoint_epoch37_1.8455819995709197.pth", help='Load model from a .pth file')
